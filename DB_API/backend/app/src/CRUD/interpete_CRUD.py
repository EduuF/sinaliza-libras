...
from backend.app.src.schemas.interprete_schema import InterpreteBase
...
class CRUDInterprete:
    def __init__(self) -> None:
        ...
        try:
            ...
            sheet_url = str(getattr(SETTINGS_VAR, 'planilha_interprete_url', None))
            worksheet_name = str(getattr(SETTINGS_VAR, 'planilha_interprete_tab_name', None))
            ...
            if not sheet_url or sheet_url.lower() == 'none':
                raise AttributeError("planilha_interprete_url não está configurado em SETTINGS_VAR.")

            if not worksheet_name or worksheet_name.lower() == 'none':
                raise AttributeError("planilha_interprete_tab_name não está configurado em SETTINGS_VAR.")
    ...
    def _get_dataframe(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Garantir que interprete_id é int para comparação
            if 'interprete_id' in df.columns:
                # Usa pd.to_numeric de forma mais robusta e eficiente
                df['interprete_id'] = pd.to_numeric(df['interprete_id'], errors='coerce').fillna(-1).astype(int)
    ...
    def get_one_by_id(self, interprete_id: int) -> Optional[InterpreteBase]:
        """Obtém o conteúdo de um interprete (interprete_id único) da planilha."""
        ...
        try:
            ...
            linha_filtrada = df[df['interprete_id'] == interprete_id]
            ...
            interprete_dict = linha_filtrada.iloc[0].to_dict()

            # Cria o modelo Pydantic
            return InterpreteBase(**interprete_dict)

        except ValueError:
            return None
        except Exception as e:
            print(f"Erro ao buscar interprete por ID: {e}")
            return None

    def delete_one_by_id(self, interprete_id: int) -> None:
        """
        Deleta um interprete (linha) pelo ID de forma PERSISTENTE no Google Sheets.
        """
        df = self._get_dataframe()
        if df is None or not self.worksheet:
            print(f"ERRO: Não foi possível conectar à planilha para deletar o ID {interprete_id}.")
            return

        try:
            # 1. Encontrar o Índice da Linha no DataFrame
            index_a_deletar = df[df['interprete_id'] == interprete_id].index

            if index_a_deletar.empty:
                print(f"AVISO: interprete com ID {interprete_id} não encontrado para deleção.")
                return
            ...
            print("-" * 50)
            print(f"SUCESSO: interprete com ID {interprete_id} deletado permanentemente da planilha (Linha {sheet_row_index}).")
            print("-" * 50)

        except Exception as e:
            print(f"Erro ao tentar deletar interprete por ID {interprete_id}: {e}")
            return

    def get_all(self) -> Optional[List[InterpreteBase]]:
        """Obtém todos os interpretes da planilha."""
        df = self._get_dataframe()
        if df is None:
            return None
        ...
        try:
            ...
            interpretes_list_dicts = df.to_dict('records')

            # ETAPA DE CORREÇÃO: Limpeza In-Place (Modifica a lista original)
            for d in interpretes_list_dicts:
            ...
            interpretes_base_list = [InterpreteBase(**d) for d in interpretes_list_dicts]

            return interpretes_base_list
        except Exception as e:
            print(f"Erro ao obter todos os interpretes: {e}")
            return None
        
    def update_one_by_id(self, interprete_id: int, column_name: str, new_value: Any) -> bool:
        """
        Atualiza o valor de uma coluna específica para um trecho (linha) usando o interprete_id.

        Args:
            interprete_id (int): O ID único do trecho a ser atualizado.
        ...
        df = self._get_dataframe()
        if df is None or not self.worksheet:
            print(f"ERRO: Não foi possível conectar à planilha para atualizar o ID {interprete_id}.")
            return False
        ...
        try:
            ...
            index_a_atualizar = df[df['interprete_id'] == interprete_id].index

            if index_a_atualizar.empty:
                print(f"AVISO: Trecho com ID {interprete_id} não encontrado para atualização.")
                return False
            ...
            print("-" * 50)
            print(
                f"SUCESSO: Trecho ID {interprete_id}: Coluna '{column_name}' atualizada para '{value_to_write}' (Linha {sheet_row_index}, Coluna {sheet_col_index}).")
            print("-" * 50)

            return True

        except Exception as e:
            print(f"Erro ao tentar atualizar trecho por ID {interprete_id}: {e}")
            return False